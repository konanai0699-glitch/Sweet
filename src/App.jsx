...
// ── Data ──────────────────────────────────────────────────────────
const CATEGORIES = [
  { id:"daily",    icon:"🧁", name:"كعك يومي",      desc:"لذيذ كل يوم",          bg:"#FFF0F5", accent:C.deepRose },
  { id:"occasion", icon:"🎂", name:"كعك مناسبات",   desc:"للأفراح والأعياد",     bg:"#FFF8ED", accent:C.gold },
  { id:"mamoul",   icon:"🌸", name:"كعك موسمي",     desc:"نكهات الموسم",         bg:"#FDF0FF", accent:"#A87BC0" },
  { id:"custom",   icon:"✨", name:"طلبات خاصة",    desc:"حسب رغبتك تماماً",     bg:"#F0FFF8", accent:"#5BAF8A" },
];
...
function ImageWithFallback({ src, alt, style, onClick, className, fallbackStyle }) {
  const [failed, setFailed] = useState(false);
  const url = `${src}.jpg`;
...
const PRODUCTS = {
  daily: [
    { id:"d1",  ...img("daily",1),  name:"كرمبو",                desc:"قاعدة بسكويت مع حشوة أجبان خفيفة مغطاة بطبقة شوكولاتة غنية", sizes:["كبير","وسط","صغير"], notes:["يُحضّر طازج يومياً","قابل للتخصيص"] },
    { id:"d2",  ...img("daily",2),  name:"تاراليتشي التركية",    desc:"كعكة خفيفة ومقرمشة بطبقات بسيطة، بتعطي قرشة لذيذة مع طعم غني وخفيف بنفس الوقت", sizes:["كبير","وسط","صغير"], notes:["يُحضّر طازج يومياً","يُطلب قبل 24 ساعة"] },
    { id:"d3",  ...img("daily",3),  name:"بسبوسة",               desc:"كعكة سميد طرية ومشبعة بالقطر، بطعم حلو غني ولمسة جوز هند أو قشطة", sizes:["كبير","وسط"], notes:["يُحضّر طازج يومياً","موسمي حسب الفاكهة"] },
    { id:"d4",  ...img("daily",4),  name:"تيراميسو",             desc:"طبقات بسكويت مغمّس بالقهوة مع كريمة ماسكربوني ناعمة ورشة كاكاو فاخرة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 24 ساعة"] },
    { id:"d5",  ...img("daily",5),  name:"كساتا",                desc:"طبقات آيس كريم ناعمة مع كيك خفيف وكريمة غنية بطعم بارد ومنعش", sizes:["كبير"], notes:["يُحضّر طازج يومياً"] },
    { id:"c7",  ...img("custom",7), name:"ليزي كيك",             desc:"كيك أسطواني بتغطية شوكولاتة داكنة وحلوى مبعثرة على السطح", sizes:["وسط"], notes:["يُطلب قبل 48 ساعة"] },
    { id:"d6",  ...img("daily",6),  name:"كعكة جوز وميبل",      desc:"كعكة طرية غنية بالجوز مع لمسة شراب ميبل حلوة ودافئة", sizes:["كبير","وسط"], notes:["يُحضّر طازج يومياً","يُطلب قبل 24 ساعة"] },
    { id:"d8",  ...img("daily",8),  name:"كعكة البرتقال",       desc:"كعكة إسفنجية طرية بنكهة البرتقال المنعشة ولمسة حلاوة خفيفة", sizes:["كبير","وسط"], notes:["يُحضّر طازج يومياً"] },
    { id:"d9",  ...img("daily",9),  name:"جبنة مخبوزة",         desc:"كعكة جبنة كريمية ناعمة مع طبقة كريما غنية ولمسة توت بري حامضة حلوة", sizes:["كبير","وسط"], notes:["يُطلب قبل 24 ساعة","موسمي"] },
    { id:"d10", ...img("daily",10), name:"كيك العسل والمكسرات", desc:"كيك ذهبي بالعسل الطبيعي مزيّن بالمكسرات المتنوعة", sizes:["كبير","وسط"], notes:["يُحضّر طازج يومياً","يُطلب قبل 24 ساعة"] },
  ],
  occasion: [
    { id:"o9",  ...img("occasion",9),  name:"كعكة كاكاو",                desc:"كعكة كاكاو هشة وغنية بنكهة الشوكلاطة اللذيذة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"o10", ...img("occasion",10), name:"بادج شوكلاطة ومكسرات",     desc:"طبقات غنية من الشوكلاطة الفاخرة مع مزيج مقرمش من المكسرات المحمصة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"o11", ...img("occasion",11), name:"بيتفور كراميل بالفستق",    desc:"قطع بيتفور ناعمة بحشوة الكراميل الغنيّة ومزيّنة بالفستق الحلبي الفاخر", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"o1",  ...img("occasion",1),  name:"كاسات بطعمات",             desc:"كاسات سفرة فاخر بشكل طولي مميز مع تغطية احترافية", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 48 ساعة","قابل للتخصيص"] },
    { id:"o2",  ...img("occasion",2),  name:"كونتيسا بيتيفور",          desc:"قطع كعك صغيرة ناعمة ومزيّنة، بطبقات خفيفة وحشوات كريمية بطعم فاخر", sizes:["صغير"], notes:["يُطلب قبل 48 ساعة","مناسب للمناسبات"] },
    { id:"o3",  ...img("occasion",3),  name:"بخزنيوت",                  desc:"كعكة شوكولاتة غنية بطبقات كريمية ناعمة وطعم فاخر ومكثّف بالتوت البري", sizes:["صغير"], notes:["يُطلب قبل 48 ساعة"] },
    { id:"o4",  ...img("occasion",4),  name:"مكعبات جبنة مخبوزة",      desc:"كعكة جبنة كريمية ناعمة تعلوها طبقة كريما مزخرفة بلمسة أنيقة وفاخرة", sizes:["صغير"], notes:["يُطلب قبل 48 ساعة","تصميم حسب الطلب"] },
    { id:"o5",  ...img("occasion",5),  name:"ماجنوم شوكلاطة",          desc:"طبقات آيس كريم ناعمة مغطاة بشوكولاتة بيضاء وبنية بطعم غني وفاخر", sizes:["صغير"], notes:["يُطلب قبل 48 ساعة"] },
    { id:"o6",  ...img("occasion",6),  name:"كريم بروليه",              desc:"كريمة ناعمة مخبوزة بسطح كراميل مقرمش ولمسة فانيلا راقية", sizes:["صغير"], notes:["يُطلب قبل 48 ساعة","موسمي"] },
    { id:"o7",  ...img("occasion",7),  name:"براونيز",                  desc:"كعكة شوكولاتة كثيفة ورطبة بطعم غني يذوب في الفم", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع"] },
    { id:"o8",  ...img("occasion",8),  name:"كونتيسا",                  desc:"قطع كعك صغيرة ناعمة ومزيّنة بحشوات خفيفة بطعم أنيق", sizes:["صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
  ],
  mamoul: [
    { id:"m1", ...img("mamoul",1), name:"كعك أصفر",                desc:"كعك طري بلون ذهبي ونكهة خفيفة بطعم بيتي لذيذ", sizes:["كبير","وسط","صغير"], notes:["يُحضّر طازج يومياً","موسم رمضان والأعياد"] },
    { id:"m2", ...img("mamoul",2), name:"مقروطة محشية راحة",       desc:"عجينة سميد طرية محشوة بالراحة بطعم حلو وقوام ناعم", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 24 ساعة","يُحضّر طازج يومياً"] },
    { id:"m3", ...img("mamoul",3), name:"كوكيز بالشوكولاطة",       desc:"كوكيز طازجة بعجينة غنية وقطع شوكولاتة بطعم لذيذ ومقرمش", sizes:["كبير","وسط","صغير"], notes:["يُحضّر طازج يومياً"] },
    { id:"m4", ...img("mamoul",4), name:"مقروطة نوتيلا وكاكاو",    desc:"مقروطة غنية محشوة بنوتيلا وكاكاو بطعم شوكولاتة كثيف", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 24 ساعة","هدية مميزة"] },
    { id:"m6", ...img("mamoul",6), name:"الفاخوروس ريبات حلاڤ",   desc:"حلوى مميزة بطبقات ناعمة مع ريبات حلاڤ غني ولمسة فاخرة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 24 ساعة"] },
    { id:"m7", ...img("mamoul",7), name:"زرد",                     desc:"معمول هشّ بطعم مميز وحشوة لذيذة تعطيه نكهة فريدة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل 24 ساعة","موسم الأعياد"] },
  ],
  custom: [
    { id:"c12", ...img("custom",12), name:"كعك خاص",             desc:"تصميم فاخر حسب ذوقك مع تفاصيل وزينة مخصصة لكل مناسبة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c13", ...img("custom",13), name:"كعك خاص",             desc:"كيك مميز بلمسات أنيقة وألوان مختارة بعناية تناسب احتفالك", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تواصل عبر واتساب"] },
    { id:"c14", ...img("custom",14), name:"كعك خاص",             desc:"تفاصيل راقية وتزيين فاخر مع إمكانية تخصيص الشكل والنكهات", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c15", ...img("custom",15), name:"كعك خاص",             desc:"كيك مخصص بالكامل ليناسب مناسبتك بلمسة فريدة وأنيقة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تواصل مباشرة عبر واتساب"] },
    { id:"c1",  ...img("custom",1),  name:"كيك الورود الوردي",   desc:"كيك أنيق مُغطّى بورود الكريمة الوردية بتصميم زهري راقٍ", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب","تواصل عبر واتساب"] },
    { id:"c2",  ...img("custom",2),  name:"كيك الفراشات والذهب", desc:"كيك مميز بالكريمة الوردية وزينة الفراشات الذهبية الفاخرة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c3",  ...img("custom",3),  name:"ليالي بيروت هلالية",  desc:"كيك أنيق بالكريمة البيضاء الناعمة وزينة الفستق الأخضر", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c4",  ...img("custom",4),  name:"كعكة جلي مميزة",      desc:"ريد فيلفيت فاخر بإطار الكريمة البيضاء وزينة التوت الأحمر", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع"] },
    { id:"c5",  ...img("custom",5),  name:"كيك أرقام مميّز",     desc:"كيك بشكل الأرقام مع طبقات الكريمة والزينة الفاخرة", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c6",  ...img("custom",6),  name:"كيك الباقة الزفافية", desc:"كيك فاخر مزيّن بباقة الورد الطبيعي للمناسبات الكبرى", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تواصل مباشرة عبر واتساب"] },
    { id:"c8",  ...img("custom",8),  name:"كيك Hello Kitty",     desc:"كيك مخصص بشخصية Hello Kitty الوردية لأصغر الأميرات", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c9",  ...img("custom",9),  name:"كيك الدبدوب الأبيض",  desc:"كيك أبيض أنيق مع دبدوب زينة وبالونات ذهبية لعيد الميلاد", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c10", ...img("custom",10), name:"كيك الزهور والورود",  desc:"كيك مزيّن بالورود الوردية الطبيعية والحمراء لمناسبات رومانسية", sizes:["كبير","وسط","صغير"], notes:["يُطلب قبل أسبوع","تصميم حسب الطلب"] },
    { id:"c11", ...img("custom",11), name:"طلب مخصص كامل",       desc:"اختر المكونات والحجم والتصميم الذي يناسبك تماماً", sizes:["كبير","وسط","صغير"], notes:["تواصل مباشرة عبر واتساب","يُطلب قبل 48 ساعة","قابل للتخصيص حسب الطلب"] },
  ],
};
...
function SplashScreen({ onDone }) {
  ...
  const DURATION = 3800;
  const SHOWCASE = SPLASH_IMAGES.slice(0, 5);
    ...
    <div className={`splash-overlay ${exiting ? "splash-exit" : ""}`}>
      ...
      <div className="splash-img-grid">
        {SPLASH_IMAGES.map((src, i) => (
          <img key={i} src={src} alt="" style={{animationDelay:`${i*0.15}s`}} />
        ))}
      </div>
      ...
      <div className="splash-content">
        {/* Logo */}
        <div className="splash-logo-ring" style={{animation:"glowPulse 2s ease-in-out infinite, splashLogo 0.8s cubic-bezier(.22,1,.36,1) 0.2s both"}}>
        ...
        <div style={{
          fontSize:13, color:"rgba(201,168,76,0.7)", fontWeight:500,
          animation:"splashTitle 0.8s cubic-bezier(.22,1,.36,1) 1.3s both",
          opacity:0, animationFillMode:"forwards", marginBottom:0,
        }}>
        ...
        <div className="splash-showcase">
          <div className="splash-cards-track">
            {SHOWCASE.map((src, i) => (
              <div key={i} className={`splash-mini-card ${i === activeCard ? "active" : ""}`}
                style={{animationDelay:`${0.4 + i*0.1}s`, animation:"splashCard 0.7s cubic-bezier(.22,1,.36,1) both", animationDelay:`${0.5+i*0.12}s`}}>
                <img src={src} alt="" />
              </div>
            ))}
          </div>
          ...
          <div className="splash-dots">
            {SHOWCASE.map((_, i) => (
              <div key={i} className={`splash-dot ${i === activeCard ? "active" : ""}`} />
            ))}
          </div>
      ...
      {/* Skip button */}
      <button className="splash-skip" onClick={() => { setExiting(true); setTimeout(onDone, 600); }}>
        اضغط للتخطي
      </button>
    </div>
  );
    ...
    <div className="lightbox-overlay" onClick={onClose}>
      ...
      {images.length > 1 && (
        <div className="lightbox-dots">
          {images.map((_, i) => (
            <div key={i} className={`lightbox-dot ${i === idx ? "active" : ""}`}
...
function SocialButtons() {
  return (
    <div style={{position:"fixed",bottom:24,left:20,zIndex:1000,display:"flex",flexDirection:"column",gap:10}}>
      <a href={WA} target="_blank" rel="noopener noreferrer" className="wa-btn btn-press" style={btnStyle("#25D366","#1aaa55")}>💬 واتساب</a>
      <a href={FB} target="_blank" rel="noopener noreferrer" className="btn-press" style={btnStyle("#1877F2","#0d5bd3")}>📘 فيسبوك</a>
      <a href={IG} target="_blank" rel="noopener noreferrer" className="btn-press" style={btnStyle("#E1306C","#C13584")}>📸 إنستغرام</a>
    </div>
  );
}
  ...
  return (
    <nav style={{
      ...
      <div style={{maxWidth:480,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:62}}>
        <button onClick={() => setPage("home")} className="btn-press"
        ...
        <div style={{display:"flex",gap:22,alignItems:"center"}}>
          <span className={`nav-link ${page==="home"?"active":""}`} onClick={() => setPage("home")}
            style={{fontSize:14,fontWeight:600,color:page==="home"?C.deepRose:C.muted}}>الرئيسية</span>
          {catId && (
            <span className={`nav-link ${page==="category"?"active":""}`} onClick={() => setPage("category")}
              style={{fontSize:14,fontWeight:600,color:page==="category"?C.deepRose:C.muted}}>المنتجات</span>
          )}
        </div>
      </div>
    </nav>
    ...
    <div className="animate-pageEnter">
      <section style={{position:"relative",minHeight:"90vh",display:"flex",flexDirection:"column",justifyContent:"center",overflow:"hidden",padding:"0 24px 48px"}}>
        ...
        <div style={{position:"relative",zIndex:1,maxWidth:480,margin:"0 auto",width:"100%",paddingTop:20}}>
          ...
          <div className="animate-fadeUp d4" style={{display:"flex",gap:12,marginTop:28,flexWrap:"wrap"}}>
            <button onClick={() => { setCat("daily"); setPage("category"); }} className="btn-press"
              style={{flex:1,background:`linear-gradient(135deg,${C.rosePink},${C.deepRose})`,color:"white",border:"none",borderRadius:50,padding:"15px 20px",fontSize:15,fontWeight:800,boxShadow:`0 8px 24px ${C.shadow}`,cursor:"pointer"}}>
              تصفّح الحلويات 🧁
            </button>
            <a href={WA} target="_blank" rel="noopener noreferrer" className="btn-press"
              style={{flex:1,background:`linear-gradient(135deg,${C.gold},${C.warmGold})`,color:"white",borderRadius:50,padding:"15px 20px",fontSize:15,fontWeight:800,textDecoration:"none",textAlign:"center",boxShadow:`0 8px 24px ${C.shadowGold}`}}>
              اطلب الآن ✨
            </a>
          </div>
      ...
      <section className="scroll-reveal" style={{margin:"0 16px 64px",borderRadius:24,padding:"32px 24px",maxWidth:448,marginLeft:"auto",marginRight:"auto",position:"relative",overflow:"hidden",background:`linear-gradient(135deg,${C.deepRose},#8B3050)`}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <a href={WA} target="_blank" rel="noopener noreferrer" className="btn-press"
            style={{display:"inline-flex",alignItems:"center",gap:8,marginTop:20,background:`linear-gradient(135deg,${C.gold},${C.warmGold})`,color:"white",borderRadius:50,padding:"12px 22px",textDecoration:"none",fontSize:14,fontWeight:800,boxShadow:"0 6px 18px rgba(0,0,0,0.2)"}}>
            📞 050-8812768
          </a>
        </div>
      </section>
    ...
    <div className="animate-pageEnter">
      <div style={{background:`linear-gradient(160deg,${cat.bg},${C.cream})`,padding:"28px 20px 24px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:480,margin:"0 auto",position:"relative"}}>
          <button onClick={() => setPage("home")} className="btn-press"
            style={{background:"white",border:`1px solid ${C.pink}`,borderRadius:50,padding:"8px 18px",fontSize:13,fontWeight:700,color:C.deepRose,cursor:"pointer",marginBottom:20,boxShadow:`0 2px 10px ${C.shadow}`}}>
            → رجوع للرئيسية
          </button>
      ...
      <div style={{padding:"16px 20px 0",maxWidth:480,margin:"0 auto"}}><GoldDivider /></div>

      <div style={{padding:"16px 16px 90px",maxWidth:480,margin:"0 auto"}}>
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          {products.map(p => (
            <div key={p.id} className="card-hover scroll-reveal"
              onClick={() => { setProduct(p); setPage("product"); }}
              style={{background:"white",borderRadius:22,overflow:"hidden",boxShadow:`0 4px 20px ${C.shadow}`,border:`1px solid ${C.pink}20`}}>
              <div style={{overflow:"hidden",height:185}}>
                <ImageWithFallback
                  src={p.main} alt={p.name}
                  style={{width:"100%",height:185,objectFit:"cover",transition:"transform 0.4s ease"}}
              ...
              <div style={{padding:"16px 18px 18px"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div style={{display:"flex",gap:6}}>
                    {ALL_SIZES.map(s => (
                      <span key={s} className={`size-tag ${p.sizes.includes(s)?"size-available":"size-unavailable"}`}
                        style={{fontSize:11,fontWeight:700,borderRadius:50,padding:"4px 11px"}}>{s}</span>
                    ))}
                  </div>
...
function ProductPage({ product, catId, setPage }) {
  ...
  const currentSrc = activeImg === "main" || !thumbExists ? product.main : product.thumb;
      ...
      <div className="animate-pageEnter">
        <div style={{position:"relative"}}>
          <div style={{height:280,overflow:"hidden",background:"#1A0F15",position:"relative"}}>
            <ImageWithFallback
              key={currentSrc}
              src={currentSrc} alt={product.name}
            ...
            <button onClick={() => setPage("category")} className="btn-press"
              style={{position:"absolute",top:18,right:18,background:"rgba(255,255,255,0.92)",border:"none",borderRadius:50,width:42,height:42,fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(0,0,0,0.2)"}}>
              →
            </button>
          ...
          {thumbExists && (
            <div className="thumb-strip">
              {[
                ...
                <button key={key} className={`thumb-btn ${activeImg===key?"active":""}`}
                  onClick={() => setActiveImg(key)} title={label}
                  style={{background:"none",border:activeImg===key?`2px solid ${C.gold}`:"2px solid transparent"}}>
                  <SmartImage
                    src={src} alt={label}
                    style={{width:"100%",height:"100%",objectFit:"cover"}}
              ...
              <div style={{flex:1}} />
              <span
                style={{fontSize:11,color:C.deepRose,alignSelf:"center",fontWeight:700,cursor:"pointer",background:C.blush,borderRadius:50,padding:"5px 12px",border:`1px solid ${C.pink}40`}}
                onClick={() => setLightbox(currentIdx)}>
                🔍 كبّر الصورة
              </span>
            </div>
          ...
          {!thumbExists && (
            <div className="thumb-strip" style={{justifyContent:"flex-end"}}>
              <span
                style={{fontSize:11,color:C.deepRose,fontWeight:700,cursor:"pointer",background:C.blush,borderRadius:50,padding:"5px 12px",border:`1px solid ${C.pink}40`}}
        ...
        <div style={{padding:"24px 20px 100px",maxWidth:480,margin:"0 auto"}}>
          ...
          <div className="animate-fadeUp d5" style={{display:"flex",gap:12}}>
            <a href={WA} target="_blank" rel="noopener noreferrer" className="btn-press"
              style={{flex:1,background:"linear-gradient(135deg,#25D366,#1aaa55)",color:"white",borderRadius:18,padding:"17px",textAlign:"center",fontSize:15,fontWeight:800,textDecoration:"none",boxShadow:"0 8px 24px rgba(37,211,102,0.4)",display:"block"}}>
              💬 اطلب عبر واتساب
            </a>
          </div>
